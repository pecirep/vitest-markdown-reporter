export const parseRemoteUrl = (url: string): { serverUrl: string; repository: string } => {
  ...
  if (url.startsWith("https://") || url.startsWith("http://")) {
    ...
    const repository = matched !== null && matched.length >= 1 ? matched[1] : undefined;
  ...
  const hostname = matched !== null && matched.length >= 2 ? matched[1] : undefined;
  const repository = matched !== null && matched.length >= 2 ? matched[2] : undefined;
